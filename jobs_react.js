

var Jobs_Array =[
   
   {
      key:'1',
      title:"UX/UI designer",
      sallary:"1200 to 1500 BGN",
      location:"Plovdiv",
      date:"Posted Today",
      pic:"images/cl1.jpg",
      time:"9 to 18 AM",
      seen:"1334",
      language:"English",
      description:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      requ:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      weoffer:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged"
   },
   {
      key:'2',
      title:"PHP developer",
      sallary:"1600 to 2200 BGN",
      location:"Sofia",
      date:"11.02.2017",
      pic:"images/cl2.jpg",
      time:"",
      seen:"",
      language:"",
      description:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      requ:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      weoffer:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged"
   },
   {
      key:'3',
      title:"Customer Support Representative - French / German",
      sallary:"",
      location:"Plovdiv",
      date:"09.02.2017",
      pic:"images/cl3.jpg",
      time:"",
      seen:"",
      language:"",
      description:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      requ:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      weoffer:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged"
   } 
   ,
   {
      key:"4",
      title:"Customer Support Representative - French / Spanish",
      sallary:"",
      location:"Plovdiv",
      date:"09.02.2017",
      pic:"images/cl6.jpg",
      time:"",
      seen:"",
      language:"",
      description:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      requ:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      weoffer:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged"
   },
   {
      key:"5",
      title:"Front End Web Developer ",
      sallary:"",
      location:"Varna",
      date:"09.02.2017",
      pic:"images/cl4.jpg",
      time:"",
      seen:"",
      language:"",
      description:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      requ:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      weoffer:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged"
   },
   {
      key:"6",
      title:"Senior C# Developer - Senior .Net Web Developer - Software Development",
      sallary:"2500 to 4500 BGN",
      location:"Sofia",
      date:"08.02.2017",
      pic:"images/cl3.jpg",
      time:"",
      seen:"",
      language:"",
      description:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      requ:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      weoffer:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged"
   },
   {
      key:"7",
      title:"Web Analyst",
      sallary:"",
      location:"Plovdiv",
      date:"08.02.2017",
      pic:"images/cl5.jpg",
      time:"",
      seen:"",
      language:"",
      description:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      requ:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      weoffer:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged"
   },
   {
      key:"8",
      title:"Web Developer - PHP/HTML/CSS - Central Manchester Agency",
      sallary:"3000 to 4000 BNG",
      location:"Sofia",
      date:"07.02.2017",
      pic:"images/cl4.jpg",
      time:"",
      seen:"",
      language:"",
      description:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      requ:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      weoffer:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged"
   },
   {
      key:"9",
      title:"Senior Marketing Executive",
      sallary:"3200 to 4200 BNG",
      location:"1400 to 1600 BGN",
      date:"07.02.2017",
      pic:"images/cl6.jpg",
      time:"",
      seen:"",
      language:"",
      description:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      requ:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      weoffer:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged"
   },
   {
      key:"10",
      title:"Middleweight Web Developer / PHP Developer / SQL Developer",
      sallary:"2200 to 2500 BGN",
      location:"Sofia",
      date:"07.02.2017",
      pic:"images/cl6.jpg"
   }
   ,
   {
      key:"11",
      title:"C#/ASP.NET MVC Web Developer",
      sallary:"1400 to 1700 BGN",
      location:"Plovdiv",
      date:"07.02.2017",
      pic:"images/cl6.jpg",
      time:"",
      seen:"",
      language:"",
      description:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      requ:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      weoffer:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged"
   }
   ,
   {
      key:"12",
      title:"Graduate Web Developer - .Net / C# / ASP.Net",
      sallary:"1600 to 1800 BGN",
      location:"Plovdiv",
      date:"07.02.2017",
      pic:"images/cl4.jpg"
   }
   ,
   {
      key:"13",
      title:"Senior Java Developer (WEB)",
      sallary:"1200 to 1500 BGN",
      location:"Sofia",
      date:"06.02.2017",
      pic:"images/cl5.jpg",
      time:"",
      seen:"",
      language:"",
      description:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      requ:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      weoffer:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged"
   }
   ,
   {
      key:"14",
      title:"Web Developer - Magento, PHP",
      sallary:"1400 to 2500 BGN",
      location:"Plovdiv",
      date:"06.02.2017",
      pic:"images/cl5.jpg",
      time:"",
      seen:"",
      language:"",
      description:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      requ:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      weoffer:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged"
   }
   ,
   {
      key:"15",
      title:"Graduate Web Developer",
      sallary:"1200 to 1500 BGN",
      location:"Sofia",
      date:"05.02.2017",
      pic:"images/cl5.jpg",
      time:"",
      seen:"",
      language:"",
      description:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      requ:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      weoffer:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged"
   }
   ,
   {
      key:"16",
      title:"Front end Web Developer",
      sallary:"1200 to 1500 BGN",
      location:"Plovdiv",
      date:"05.02.2017",
      pic:"images/cl4.jpg",
      time:"",
      seen:"",
      language:"",
      description:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      requ:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      weoffer:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged"
   }
   ,
   {
      key:"17",
      title:".Net Web Developer",
      sallary:"1200 to 1500 BGN",
      location:"Plovdiv",
      date:"04.02.2017",
      pic:"images/cl4.jpg",
      time:"",
      seen:"",
      language:"",
      description:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      requ:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged",
      weoffer:"Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged"
   }



];


$(function () {
var firstVisit_count = 0;

   for (var i = 0; i < Jobs_Array.length; i++){
   console.log(i);
   }
   
   $(".job_count").text( i+ " found");

   $(".all_job_container a").click(function(event) {
      
      var targetID = $(this).attr('data-id');
      var targetID_number = parseInt(targetID.substring(4));
      /*the index(key) of the element*/
      var selected_job = Jobs_Array[targetID_number-1];
      console.log(selected_job.key +"  "+selected_job.title);
    
      //$(".all_job_container").hide(); 
      $(".all_job_container").addClass('all_job_container_unscrollable');
      $(".single_job_container_master ").show(); 

      $(".jb_pic_data").attr("src", selected_job.pic);
      $(".jb_title_data").text(selected_job.title);
      $(".jb_seen_data").text(selected_job.date);
      $(".jb_place_data").text(selected_job.location);
      $(".jb_time_data").text(selected_job.time);
      $(".jb_sallary_data").text(selected_job.sallary);
      $(".jb_lang_data").text(selected_job.language);
      $(".jb_des_data").text(selected_job.description);
      $(".jb_req_data").text(selected_job.requ);
      $(".jb_wo_data").text(selected_job.weoffer);
       ++firstVisit_count;
      if (firstVisit_count ==2) {
         $(".jb_return_message").hide();
      }

   });

   $("#return_to_jobs").click(function () {

    $(".single_job_container_master ").fadeOut(100, function() {
       $(".single_job_wraper").animate({ scrollTop: 0 }, 100);
    });
    
      






   });





});







var JobsList = React.createClass({
	

 
  
   
	renderAll:function() {
		const data = this.props.data;

				const jobList = data.map(job=>{

					return(
							<a 
                         className="job_link_a" data-id={"job_"+job.key} key={job.key}><div className="job_container">
   								<div className="job_text">
   									<p className="job_title">{job.title}</p>
   									
   									<p className="job_sallary">{job.sallary}</p>
   									<p className="job_location">{job.location}</p>
   								</div>

   								<div className="job_image_container">
   									<p className="job_date">{job.date}</p>
   									<img className="job_image" src={job.pic}/>

   								</div>
   							</div>
                     </a>
							
							)
					});
				return jobList;
	},
   render:function () {
		
		return (
         <div>
            <div className="main_page_title_container">
               <p className="main_page_title job_title_page">Jobs</p>
               <p className="job_count"></p>  
            </div>
            <div className="loader_conateiner"><div className="pre_loader"></div></div>

            <div className="all_job_container">
               <div>{this.renderAll()}</div>
            </div>
            <div className="single_job_container_master">
               <div className="single_job_container">
                  <div className="single_job_wraper">
                       
                        <button className="jb_return_btn" id="return_to_jobs"><span>&laquo;</span></button>
                       
                     <div className="div_flex_center"><img src="" className="jb_pic_data"/> </div>
                     <p className="jb_title_data"></p>
                     <div className="div_flex_center"><hr className="jb_hr"/>  </div>
                     <div className="">
                        <p className="jb_details_p">Seen: <span className="jb_details_span  jb_seen_data"></span></p>
                        <p className="jb_details_p">Place: <span className="jb_details_span  jb_place_data"></span></p>
                        <p className="jb_details_p">Time: <span className="jb_details_span  jb_time_data"></span></p>
                        <p className="jb_details_p">Sallary: <span className="jb_details_span  jb_sallary_data"></span></p>
                        <p className="jb_details_p">Languages: <span className="jb_details_span jb_lang_data"></span></p>
                     </div>
                        <div className="div_flex_center"><hr className="jb_hr"/>  </div>
                     <div className="">
                        <p className="jb_section">Description:</p>
                           <p className="jb_des_data jb_section_data"></p>
                         <p className="jb_section">Recuerments:</p>
                           <p className="jb_req_data jb_section_data"></p>
                         <p className="jb_section">We Offer:</p>
                           <p className="jb_wo_data jb_section_data"></p>

                     </div>
                     <div  className="div_flex_center">
                        <button className="jb_apply_btn input button_input">Apply</button>
                        
                     </div>
                   
                     
                    

                     </div>
               </div>
            </div>

         </div>
			
			)
		}

});



ReactDOM.render( <JobsList data={Jobs_Array}/> , document.getElementById('jobs_react'));

